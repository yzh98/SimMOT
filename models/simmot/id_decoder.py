...
class IDDecoder(nn.Module):
    ...
    def forward(self, seq_info, use_decoder_checkpoint):
        ...
        # Note: _L <= _curr_L after conducting self.simmot.
        _B, _G, _T, _L, _N, _ = trajectory_features.shape
        _curr_B, _curr_G, _curr_L, _curr_N, _ = unknown_features.shape
        ...
        # Note: Temporal mask is added during conduct "self.simmot()"
        cross_attn_key_padding_mask = einops.rearrange(trajectory_masks, "b g t l n -> (b g t) (l n)").contiguous()
        _trajectory_times_flatten = einops.rearrange(trajectory_times, "b g t l n -> (b g t) (l n)")
        _unknown_times_flatten = einops.rearrange(unknown_times, "b g t n -> (b g t) n")
        cross_attn_mask = None  # we split temporal information to batch dimension, therefore no need for cross_attn_mask
        # Prepare for rel PE:
        ...
        rel_pe_idxs = self.rel_pos_map[rel_pe_idx_pairs[..., 0], rel_pe_idx_pairs[..., 1]]      # (B*G, L_curr, L_traj)
        pass
    ...
    def _forward_a_layer(
        ...
        _B, _G, _T, _L, _N, _ = trajectory_embeds.shape
        ...
        cross_trajectory_embeds = einops.rearrange(trajectory_embeds, "b g t l n c -> (b g t) (l n) c").contiguous()
        # Prepare attn_mask:
        rel_pe_mask = self.rel_pos_embeds[layer][rel_pe_idx]
        rel_pe_mask = einops.rearrange(rel_pe_mask, "bgt l1 l2 n -> (bgt n) l1 l2")
