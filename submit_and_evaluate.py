...
def submit_and_evaluate(config: dict):
    ...
    model, _ = build_simmot(config=config)
...
def submit_and_evaluate_one_model(
    ...
    if not is_evaluate:
        ...
        if accelerator.is_main_process:
            ...
            prj_abs_dir = os.path.dirname(os.path.abspath(__file__))
            ...
            if dataset in ["DanceTrack", "SportsMOT", "BFT"] or (dataset in ["MOT17"] and data_split == "test"):
                ...
                cmd = ["python", f"{prj_abs_dir}/TrackEval/scripts/run_mot_challenge.py"]
            elif dataset in ["PersonPath22_Inference"]:
                args = {
                    "--SPLIT_TO_EVAL": data_split,
                    "--METRICS": ["HOTA", "CLEAR", "Identity"],
                ...
                cmd = ["python", f"{prj_abs_dir}/TrackEval/scripts/run_person_path_22.py"]
            else:
