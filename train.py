...
from models.simmot import build as build_simmot
from models.simmot.id_criterion import build as build_id_criterion
...
def train_engine(config: dict):
    ...
    # Build SimMOT model:
    model, detr_criterion = build_simmot(config=config)
    ...
    if config["DETR_NUM_TRAIN_FRAMES"] == 0:
        for n, p in model.named_parameters():
            if "detr" in n:
                p.requires_grad = False     # only train the SimMOT part.
...
def train_one_epoch(
    ...
    for step, samples in enumerate(dataloader):
        ...
        # Whether to only train the DETR, OR to train the SimMOT together:
        if not only_detr:
            _G, _, _N = annotations[0][0]["trajectory_id_labels"].shape
            # Need to prepare for SimMOT:
            seq_info = prepare_for_simmot(
...
def prepare_for_simmot(detr_outputs, annotations, detr_indices):
    _B, _T = len(annotations), len(annotations[0])
    _G, _, _N = annotations[0][0]["trajectory_id_labels"].shape
